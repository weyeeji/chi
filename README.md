...
> 想先用大白话和具体例子看懂整个实验（目的、参与者经历、数据、分析），请直接读 `Proposal.md` 的 **§0 实验全景**。下面 §1.5 是浓缩版。

## 1.5 实验一图读懂（具体例子）

**目的（大白话）**：多智能体 AI 产品里，给每个 agent 设计区分化的语气风格（组织者有条理、发想者发散、批评者爱挑刺、验证者谨慎）值不值得？还是中性语气就够、甚至带性格反而有压力？

**怎么比**：同一个人先后和两支团队各做一个设计任务——A 队全中性语气、B 队带角色特定性格，其余全相同。同一个人做两次=自己对照，~20 人即可分析。

**两队差在哪（同一角色的真实回复，取自 `server/agent-personalities.json`）**：

```text
Critic 批评者
  A 队(中性): "一个可能的局限是，仅靠提醒不一定改变行为。可以在方案里稍作考虑。"
  B 队(特定): "这里最大的风险是把提醒等同于行为改变。高压力情境下用户可能跳过提示，
              因此需要设计能改变决策结构的机制。"
两队内容相近(防"B队只是说得更多")，变的是表达强度。
```

**参与者经历（约 50–60 分钟，全程网页）**：

```text
同意+前问卷 → 第一轮(A队,题目1): 独立写初稿 → 角色说明 → 3轮团队协作(中途处理4条建议:2好2坏)
            → 写终稿 → 本轮问卷
          → 第二轮(B队,题目2): 写初稿 → 3轮协作 → 终稿 → 本轮问卷
          → 两轮对比问卷 → debrief(揭示哪些是故意设计的坏建议)
参与者全程不知道两队区别是"性格"，也不知道哪些建议是陷阱。
```

**每人产出的数据**：4 份方案(2轮×初稿/终稿)、每轮 4 条建议的处理选择、2 份本轮问卷、对话日志、对比问卷——每项都有 A 队版和 B 队版，可配对。

**怎么算出结论（被试内配对）**：

```text
1. 盲评：4 份方案拆成匿名单项，3 位评分者不知队别/初终稿，按7维度打1–7分。
2. 每人算: 改进量=终稿均分−初稿均分; 校准分=4条建议处理正确的比例。
3. 看每个人的"B队−A队"配对差，整体是否显著>0：
     改进量    B−A=+0.52, Cohen's dz=0.85, Wilcoxon p=0.012  ✅
     校准依赖  B−A=+0.30, dz=0.72                            ✅
     角色可辨识 B−A=+0.90, dz=1.10                           ✅
     感知压力  B−A=+0.65, dz=0.68                            ⚠️ 代价
   (以上为假数据示意)
→ 故事不是"带性格全面更好"，而是"提升产出与判断力，但代价是更高压力"。
```

（上述配对统计由 `/admin` 仪表盘和 `npm run audit:study1` 自动算出。）
